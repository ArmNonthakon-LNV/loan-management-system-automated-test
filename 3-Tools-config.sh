...
# Usage:
#   ./3-Tools-config.sh [TARGET_DIRECTORY]
# If TARGET_DIRECTORY is not provided, the current directory is used.
#
# This script writes tools (templates) into:
#   <TARGET_DIRECTORY>/cypress/tools/

TARGET_DIR="${1:-$(pwd)}"
TOOLS_DIR="$TARGET_DIR/cypress/tools"

echo "Writing Cypress tools into: $TOOLS_DIR"

mkdir -p "$TOOLS_DIR"

cat > "$TOOLS_DIR/recordTestEvident.js" << 'EOF'
...
echo "Done. Tools files written:"
echo " - $TOOLS_DIR/recordTestEvident.js"
